package utils;

import java.util.Map;
import java.util.Stack;

public class Eval {
    private Map<String, Double> variables;

    public Map<String, Double> getVariables() {
        return variables;
    }

    public void setVariables(Map<String, Double> variables) {
        this.variables = variables;
    }

    public Double evalExpression(String expression) {

        char[] tokens = expression.toCharArray();

        Stack<Double> values = new Stack<>();
        Stack<Character> operators = new Stack<>();

        for (int i = 0; i < tokens.length; i++) {
            if (tokens[i] == ' ') {
                continue;
            }

            if ((tokens[i] >= '0' && tokens[i] <= '9') || tokens[i] == '.') {
                String number = "";

                while (i < tokens.length && (Character.isDigit(tokens[i]) || tokens[i] == '.')) {
                    number = number + tokens[i];
                    i++;
                }
                values.push(Double.parseDouble(number));
            }

            else if (tokens[i] == '(') {
                operators.push(tokens[i]);

            } else if (tokens[i] == ')') {
                while (operators.peek() != '(') {
                    values.push(evalOperator(values.pop(), values.pop(), operators.pop()));
                }

                operators.pop();
            }

            else if (tokens[i] == '+' || tokens[i] == '-'
                    || tokens[i] == '*'
                    || tokens[i] == '/') {
                while (!operators.isEmpty()
                        && hasPrecedence(tokens[i],
                                operators.peek())) {
                    values.push(evalOperator(
                            values.pop(),
                            values.pop(), operators.pop()));
                }
                operators.push(tokens[i]);

            }

        }
        while (!operators.isEmpty()) {
            values.push(evalOperator(
                    values.pop(),
                    values.pop(),
                    operators.pop()));
        }
        return values.pop();
    }

    private static boolean hasPrecedence(char operator1,
            char operator2) {
        if (operator2 == '(' || operator2 == ')')
            return false;
        return (operator1 != '*' && operator1 != '/')
                || (operator2 != '+' && operator2 != '-');
    }

    private Double evalOperator(double b, double a, char operator) {
        switch (operator) {
            case '+':
                return a + b;
            case '-':
                return a - b;
            case '*':
                return a * b;
            case '/':
                if (b == 0)
                    throw new ArithmeticException("Cannot divide by zero");
                return a / b;
        }

        return 0d;
    }

    public static void main(String[] args) {

        Eval eval = new Eval();
        System.out.println(eval.evalExpression("100.10 + 12"));

    }

}
